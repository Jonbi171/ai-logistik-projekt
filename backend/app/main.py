...
def get_current_inventory(
    ...
    statuses = dict(
        session.execute(
            select(view.c.inventory_status, func.count())
            .group_by(view.c.inventory_status)
        ).all()
    )
...
def get_shipment_overview(
    ...
    ).order_by(view.c.planned_arrival_at.desc()).limit(limit)
    results = session.execute(statement).mappings().all()
    rows = [
        {
    ...
    return overview_response(
        ...
        rows,
        {
            ...
            "shipment_delay_rate_percent": percentage(
                metrics["delayed"], metrics["total_records"]
            ),
...
def get_sales_order_overview(
    ...
    return overview_response(
        ...
        rows,
        {
            ...
            "order_fulfillment_rate_percent": percentage(
                metrics["delivered"], metrics["ordered"]
            ),
            "late_orders": metrics["late_orders"],
            "late_order_rate_percent": percentage(
                metrics["late_orders"], metrics["total_orders"]
            ),
        },
        metrics["total_records"],
    )
...
def get_purchase_order_overview(
    ...
    metrics = session.execute(
        select(
            ...
            func.count()
            .filter(order_metrics.c.supplier_on_time.is_(True))
            .label("on_time"),
            func.coalesce(func.avg(order_metrics.c.actual_lead_time_days), 0)
            .label("average_lead_time"),
    ...
    return overview_response(
        ...
        rows,
        {
            ...
            "average_supplier_lead_time_days": round(
                float(metrics["average_lead_time"]), 2
            ),
...
def get_inventory_history(
    ...
    statement = select(
        ...
        view.c.lot_number,
        view.c.expiry_date,
    ).order_by(view.c.event_timestamp.desc()).limit(limit)
    results = session.execute(statement).mappings().all()
    rows = [
        {
    ...
    movement_types = dict(
        session.execute(
            select(view.c.movement_type, func.count())
            .group_by(view.c.movement_type)
        ).all()
    )
    return overview_response(
...
def get_product_bom(
    ...
    metrics = session.execute(
        select(
            func.count().label("total_records"),
            func.count(func.distinct(view.c.finished_product_sku)).label(
                "finished_products"
            ),
            func.count().filter(view.c.scrap_factor > 0).label("scrap_lines"),
            func.count().filter(view.c.effective_to < date.today()).label(
                "expired_lines"
            ),
        )
    ).mappings().one()
...
def get_product_supplier_overview(
    ...
    risk_tiers = dict(
        session.execute(
            select(view.c.risk_tier, func.count()).group_by(view.c.risk_tier)
        ).all()
    )
    return overview_response(
        ...
        rows,
        {
            ...
            "average_contracted_lead_time_days": round(
                float(metrics["average_lead_time"]), 2
            ),
...
def get_shipment_events(
    ...
    statement = select(
        ...
        view.c.city,
        view.c.country_code,
    ).order_by(view.c.event_timestamp.desc(), view.c.shipment_number).limit(limit)
    results = session.execute(statement).mappings().all()
    rows = [
        {
            "shipment_number": row["shipment_number"],
    ...
    event_types = dict(
        session.execute(
            select(view.c.event_type, func.count()).group_by(view.c.event_type)
        ).all()
    )
    return overview_response(
        "vw_shipment_events",
